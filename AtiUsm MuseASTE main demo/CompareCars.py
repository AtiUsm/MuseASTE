...
def app():
    ...
    x = st.sidebar.feedback("thumbs", key='compf')
    if x == 1:
        st.balloons()

    st.sidebar.pills("Tags", ["ASTE", "Knowledge Graph", "Aspect Sentiment Triplet Extraction","Aspect Based Knowledge Graphs"], key='compp')
    ...
    if criterion== 'Topic':
        ...
        topicn=st.selectbox('Choose Topic', topicnames, key='comTop', index=3)
    ...
    if criterion=='Sentiment':
        ...
        with col1:
            ...
            for topic in range(0,10,1):
                triples=get_triples(topic,df, st.session_state["car_1"],sentiment='pos')
                p1=p1+len(triples)
                triples=get_triples(topic,df,st.session_state["car_1"],sentiment='neg')
                n1=n1+len(triples)
            
            st.header('Total number of positive and negative triples in an entity') 
            sentiment_distribution(p1,n1, key='sentallarrange1')
        ...
        with col2:
            ...
            n2=0
            for topic in range(0,10,1):
                triples=get_triples(topic,df,st.session_state["car_2"],sentiment='pos')
                p2=p2+len(triples)
                triples=get_triples(topic,df,st.session_state["car_2"],sentiment='neg')
                n2=n2+len(triples)
            
            st.header('Total number of positive and negative triples in an entity') 
            sentiment_distribution(p2,n2, key='sentallarrange2')
            st.header('Negatives')
    ...
    if criterion=='Aspect':
        ...
        df['triple'] = df.apply(lambda x: construct_triple(x.aspect, x.opinion,x.sentiment), axis=1)
        # Search bar
        search_term = st.text_input("Search for an aspect:", key='aspect1')
        with col1:
            #st.title('CAR36')
            ...
            # Dropdown search
            selected_aspect = st.selectbox("Select an aspect:",aspects,key='select_box1')

            


            # Determine which sentiments to include
            selected_sentiments = ['pos', 'neg']
        ...
        with col2:
            ...
            # Dropdown search
            selected_aspect = st.selectbox("Select an aspect:",aspects,key='select_box2')
