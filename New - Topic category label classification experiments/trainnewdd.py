...
class TextDataset(Dataset):
    def __init__(self, texts, labels, tokenizer, secondary_cols=None, max_length=200):
        self.texts = texts
        self.labels = labels
...
# ---------------------------
def plot_confusion_matrix(y_true, y_pred, normalize=False, title=None, ticklabels=None, path=None, cmap=plt.cm.Blues):
    cm = confusion_matrix(y_true, y_pred)
    if normalize:
        cm = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis]
    classes = ticklabels if ticklabels else unique_labels(y_true, y_pred)
...
def export_predictions(output_dir, y_true, y_pred, name):
    ...
    with open(os.path.join(output_dir, f"{name}_y.pkl"), "wb") as f:
        pickle.dump(y_true, f)
    with open(os.path.join(output_dir, f"{name}_y_pred.pkl"), "wb") as f:
...
def classification_results(output_dir, y_true, y_pred, class_names, partition_name, writer=None):
    os.makedirs(output_dir, exist_ok=True)
    f1_score_val = f1_metric(y_true, y_pred)
    uar_score_val = uar_metric(y_true, y_pred)
    topic_score = 0.66*f1_score_val + 0.34*uar_score_val
    print(f"{partition_name} Metrics: F1={f1_score_val}, UAR={uar_score_val}, TopicScore={topic_score}")
    ...
    plot_confusion_matrix(y_true, y_pred, ticklabels=class_names, title=f"Confusion Matrix {partition_name}", path=output_dir)
    ...
    with open(metrics_file, "w", newline="") as f:
        ...
        csv_writer.writerow(['F1','UAR','Topic_Score'])
        csv_writer.writerow([f1_score_val, uar_score_val, topic_score])

    # TensorBoard
    if writer:
        writer.add_scalar(f'{partition_name}/F1', f1_score_val)
        writer.add_scalar(f'{partition_name}/UAR', uar_score_val)
        writer.add_scalar(f'{partition_name}/TopicScore', topic_score)

# ---------------------------
# Main worker
# ---------------------------
def main_worker(rank, world_size, train_mode=True, FINAL_EVAL_MODE=True):
    free_gpu_cache(rank)
    ...
    torch.manual_seed(Param_local['manual_seed'])
    ...
    train_df = pd.read_csv("train136.csv", encoding="latin1", usecols=fields)
    devel_df = pd.read_csv("devel45.csv", encoding="latin1", usecols=fields)
    test_file = "test46.csv"

    def load_columns(df):
        texts = df['review'].tolist()
        labels = df['label_topic'].tolist()
        secondary = {col: df[col].tolist() for col in ['aspect','opinion','sentiment'] if col in df.columns}
        return texts, labels, secondary
    ...
    if not train_mode:
        best_model_dir = Param_local['best_model']
            ...
            for step, batch in enumerate(pbar):
                ...
                if (step+1) % Param_local['gradient_accumulation_steps'] == 0:
                    ...
                    if Param_local['evaluate_during_training'] and global_step % Param_local['evaluate_during_training_steps']==0:
                        ...
                        class_names = pd.read_csv("topic_class_mapping_MuSe2020.csv")['topic'].tolist()
    ...
    if FINAL_EVAL_MODE:
        ...
        for loader, name in zip(
            [train_loader,dev_loader] + ([test_loader] if x_test else []),
            ["train","dev"] + (["test"] if x_test else [])
        ):
...
if __name__ == "__main__":
    ...
    # Train flag — default True, turn OFF if you pass --no_train
    parser.add_argument(
        '--no_train',
        action='store_false',
        dest='train',
        help='Skip training; by default training runs'
    )

    # Final evaluation flag — default True, turn OFF if you pass --no_eval_final
    parser.add_argument(
        '--no_eval_final',
        action='store_false',
        dest='eval_final',
        help='Skip final evaluation; by default final evaluation runs'
    )

    args = parser.parse_args()

    # Assign to variables for clarity
    TRAIN_MODE = args.train          # True by default
    FINAL_EVAL_MODE = args.eval_final  # True by default

    print(f"TRAIN_MODE = {TRAIN_MODE}, FINAL_EVAL_MODE = {FINAL_EVAL_MODE}")
    ...
    torch.multiprocessing.spawn(
        main_worker,
        args=(world_size, TRAIN_MODE, FINAL_EVAL_MODE),
        nprocs=world_size,
        join=True
    )