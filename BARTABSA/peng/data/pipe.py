...
class BartBPEABSAPipe(Pipe):
    def __init__(self, tokenizer='facebook/bart-base', opinion_first=True):
        ...
        for key, value in self.mapping.items():
            key_id = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(value, truncation=True))
    ...
    def process(self, data_bundle: DataBundle) -> DataBundle:
        ...
        def prepare_target(ins):
            ...
            for word in raw_words[:800]:
                bpes = self.tokenizer.tokenize(word, add_prefix_space=True, truncation=True)
