...
class Seq2SeqSpanMetric(MetricBase):
    ...
    def evaluate(self, target_span, pred, tgt_tokens):
        ...
        for i, (ts, ps) in enumerate(zip(target_span, pred.tolist())):
            ...
            o_target=[tuple(t[:2]) for t in ts]
            o_pred=[p[:2] for p in pairs]
             
            a_target=[tuple(t[2:4]) for t in ts]
            a_pred=[p[2:4] for p in pairs]
            ...
            o_target_counter = Counter(o_target)
            o_pred_counter = Counter(o_pred)
            tp, fn, fp = _compute_tp_fn_fp(set(list(o_pred_counter.keys())),
                                           set(list(o_target_counter.keys())))
            ...
            a_target_counter = Counter(a_target)
            a_pred_counter = Counter(a_pred)
            tp, fn, fp = _compute_tp_fn_fp(set(list(a_pred_counter.keys())),
                                           set(list(a_target_counter.keys())))
    ...
    def get_metric(self, reset=True):
        ...
        f, pre, rec = _compute_f_pre_rec(1, self.a_tp, self.a_fn, self.a_fp)
        res["a_f"] = round(f, 4)*100
        res["a_rec"] = round(rec, 4)*100
        res["a_pre"] = round(pre, 4)*100

        f, pre, rec = _compute_f_pre_rec(1, self.o_tp, self.o_fn, self.o_fp)
        res["o_f"] = round(f, 4)*100
        res["o_rec"] = round(rec, 4)*100
        res["o_pre"] = round(pre, 4)*100
