...
from fastNLP.core.sampler import SortedSampler, ConstTokenNumSampler
...
def con(x):
    if x['src_seq_len']==1553:
        #print(' '.join(x['raw_words']))
        return True
    return False

x=source_data_bundle.get_dataset('train')
...
if args.mode == 'train':
    ...
    trainer = Trainer(train_data=x, model=model, optimizer=optimizer,
                  loss=Seq2SeqLoss(), accumulation_steps=4,device=7,driver='torch',
    ...
    tester = Tester(data=target_data_bundle.get_dataset('test'), model=model, metrics=metric, batch_size=args.batch_size,
                num_workers=2, device=device, accelerator='auto', strategy='auto', use_tqdm=True, callbacks=callbacks)
    res = tester.test()
    ...
    examples = []
    with open(f'triplet_data/{target}/test.txt') as f:
        for line in f:
