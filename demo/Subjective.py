...
def app():
    ...
    tab0,tab01,tab1, tab2 , tab3, tab4, tab5, tab6= st.tabs(["Structural Summary", "View Structural Entity Graph","Arrange by Topic", "Arrange by Sentiment","View Topic Entity Graph","Arrange By Aspect", "Review Opinion", "Source and Credibility Metrics"])
    ...
    fields=["id", "label_topic", "aspect","opinion","sentiment", "category"]
    cf2=pd.read_csv("AspectCategoryLabels.csv")
    cf1=pd.read_csv("datasetannotations.csv",usecols=fields)
    #cf=pd.read_csv("fullannotations.csv",usecols=fields)
    cf = cf1.merge(
        cf2[['aspect', 'base_category', 'MAIN_CATEGORY']],
        left_on='aspect',
        right_on='aspect',
        how='inner'
        )
    ...
    with tab0:
        ...
        for MAIN_CATEGORY in subset["MAIN_CATEGORY"].unique():
            ...
            # Only base categories under this main category
            base_categories = subset[subset["MAIN_CATEGORY"] == MAIN_CATEGORY]["base_category"].unique()
    ...
    with tab01:
        ...
        selected_main_categories = st.multiselect(
            "Select Base Categories to Display",
        ...
        if "All" in selected_main_categories and len(selected_main_categories)>1:    
            selected_main_categories=selected_main_categories.remove("All")
        else:
            selected_main_categories_filtered = [x for x in selected_main_categories if x!="All" ] 

        # --- Base categories dynamically based on selected main categories ---
        base_categories = subset[subset["MAIN_CATEGORY"].isin(selected_main_categories_filtered)]["base_category"].unique().tolist()
        ...
        G, fig = base_entity_graph(car_id, cf, density, selected_main_categories_filtered, final_base_selection)
        st.pyplot(fig)
                                    ...
                                    with row[j]:
                                                ...
                                                for _, row_topic in subset_aspect.iterrows():
                                                            ...
                                                            if topic not in topic_dict:
                                                                        topic_dict[topic] = []
                                                            topic_dict[topic].append((opinion, sentiment))

                                                # Build HTML card
                                                html_card = f"<div style='border:1px solid #ddd; padding:10px; border-radius:5px; margin-bottom:10px;'>"
                                                ...
                                                # Render opinions grouped by topic
                                                for topic, opinions_list in topic_dict.items():
                                                            html_card += f"<div style='font-size:11px; color:#444; margin-top:5px;'>{topic}:</div>"
                                                            html_card += "<div style='display:flex; flex-wrap:wrap; gap:4px;'>"
                                                            for opinion, sentiment in opinions_list:
                                                                        color = {"pos": "#afa", "neg": "#faa", "neu": "#add8e6"}.get(sentiment, "#fff")
                                                                        html_card += f"<span style='background-color:{color}; padding:2px 6px; border-radius:3px; white-space:nowrap;'>{opinion}</span>"
                                                            html_card += "</div>"  # end opinions div

                                                html_card += "</div>"  # end card
