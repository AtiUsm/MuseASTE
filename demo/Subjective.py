...
def app():
    ...
    tab4, tab5,tab0,tab1, tab2 , tab01,tab3,  tab6= st.tabs(["Arrange By Aspect", "Review Opinion", "View By Base Category","Arrange by Topic", "Arrange by Sentiment","View Structural Entity Graph","View Topic Entity Graph", "Source and Credibility Metrics"])
    ...
    df=pd.read_csv("datasetannotations.csv", usecols=fields, encoding="latin1") #give the link to train file annotations
    ...
    #cf=pd.read_csv("fullannotations.csv",usecols=fields)
    ...
    def base_entity_graphOLD(id, cf, density, selected_main_categories, selected_base_categories):
        color={'pos':'green','neg':'red'}
        G = nx.DiGraph()
        
        subset = cf[(cf["id"] == car_id) & (cf["sentiment"] != '-')]
        count=0
        for MAIN_CATEGORY in selected_main_categories:
               
            
            for category in selected_base_categories:
               
                subcf = subset[
                    (subset["MAIN_CATEGORY"] == MAIN_CATEGORY) & 
                    (subset["base_category"] == category) & 
                    (subset["sentiment"] != "neu")
                    ]
                triples = subcf["triple"].tolist()
                count=count+len(triples)
                if len(triples)==0:
                    continue
                G.add_node("car"+str(id), color='pink', edgecolor='black', position='left')
                G.add_node(MAIN_CATEGORY, color='gray', edgecolor='black', position='left')
                # Only base categories under this main category
                G.add_edge("car"+str(id), MAIN_CATEGORY, color='black', edgecolor='black')
                max_nodes = 0
                for aspect, opinion, sentiment in triples:
                    G.add_node(category, color='blue', edgecolor='blue', position='center')
                    # Only add aspect node & edge if it's NOT the same as category
                    G.add_node(opinion, color=color.get(sentiment), edgecolor='black', position='none')
                    if aspect != category:
                        G.add_node(aspect, color='yellow', edgecolor='blue', position='none')
                        G.add_edge(category, aspect, color='blue')
                        G.add_edge(aspect, opinion, label=sentiment, color=color.get(sentiment))
                    else:
                        # Directly connect category -> opinion if aspect == category
                        G.add_edge(category, opinion, label=sentiment, color=color.get(sentiment))
                    max_nodes += 1
                    if max_nodes >= density:
                        break
                G.add_edge(MAIN_CATEGORY, category, color='black', edgecolor='black')
        ...
        if count!=0:
            ...
            st.header("No triples found.")
        ax.axis('off')

        return G, fig
  
    #st.segmented_control("Filter", ["Open", "Closed"])    
    def base_entity_graphWorking(id, cf, density, selected_main_categories, selected_base_categories):
        color = {'pos':'green', 'neg':'red'}
        G = nx.DiGraph()

        subset = cf[(cf["id"] == id) & (cf["sentiment"] != '-')]

        count = 0
        car_node = f"car{id}"
        if car_node not in G.nodes:
            G.add_node(car_node, color='pink', edgecolor='black', position='left')

        for MAIN_CATEGORY in selected_main_categories:
            if MAIN_CATEGORY not in G.nodes:
                G.add_node(MAIN_CATEGORY, color='gray', edgecolor='black', position='left')
                G.add_edge(car_node, MAIN_CATEGORY, color='black', edgecolor='black')

            main_subset = subset[subset["MAIN_CATEGORY"] == MAIN_CATEGORY]

            # Filter only rows with base categories in the selected_base_categories
            def split_base_categories(row):
                return [c.strip() for c in row["base_category"].split(",")]

            main_subset = main_subset[
                main_subset.apply(lambda row: bool(set(split_base_categories(row)) & set(selected_base_categories)), axis=1)
            ]

            for _, row in main_subset.iterrows():
                triples = [row["triple"]]
                triple_base_cats = [c.strip() for c in row["base_category"].split(",")]

                # Only keep categories that are in selected_base_categories
                triple_base_cats = [c for c in triple_base_cats if c in selected_base_categories]

                if not triple_base_cats:
                    continue  # skip if none match selection

                # Add base category nodes
                for category in triple_base_cats:
                    if category not in G.nodes:
                        G.add_node(category, color='blue', edgecolor='blue', position='center')
                        G.add_edge(MAIN_CATEGORY, category, color='black', edgecolor='black')

                # Add triples under this base category
                for aspect, opinion, sentiment in triples:
                    G.add_node(opinion, color=color.get(sentiment), edgecolor='black', position='none')

                    for category in triple_base_cats:
                        if aspect != category:
                            if aspect not in G.nodes:
                                G.add_node(aspect, color='yellow', edgecolor='blue', position='none')
                            G.add_edge(category, aspect, color='blue')
                            G.add_edge(aspect, opinion, label=sentiment, color=color.get(sentiment))
                        else:
        ...
        nodes = G.nodes()
        edges = G.edges()
        colors = [G.nodes[n].get('color', 'gray') or 'gray' for n in nodes]
        edgecolors = [G.edges[n].get('color', 'black') or 'black' for n in edges]
        borders = [G.nodes[n].get('edgecolor', 'black') or 'black' for n in nodes]


        fig, ax = plt.subplots(figsize=(25, 25), dpi=300)
        pos = nx.kamada_kawai_layout(G)
        label_options = {"ec": "k", "fc": "white", "alpha": 0.7}
        nx.draw_networkx_nodes(G, pos, node_size=10000, node_color=colors, node_shape='o',
                               edgecolors=borders, linewidths=3, ax=ax)
        nx.draw_networkx_edges(G, pos, edge_color=edgecolors, edgelist=G.edges(), width=3, ax=ax)
        nx.draw_networkx_labels(G, pos, font_size=20, bbox=label_options, ax=ax)
        edge_labels = nx.get_edge_attributes(G, 'label')
        ...
        ax.axis('off')
        return G, fig
    def base_entity_graph(id, cf, density, selected_main_categories, selected_base_categories):
        color = {'pos':'green','neg':'red'}
        G = nx.DiGraph()
        ...
        for MAIN_CATEGORY in selected_main_categories:
            ...
            for _, row in main_subset.iterrows():
                ...
                # ----- EXTRA Opinion Base Nodes -----
                extra_values = row.get("EXTRA_Opinion_Base_Needed_RARE_EXAMPLE", "FALSE")
                if extra_values != "FALSE" and extra_values:
                    for extra_value in [v.strip() for v in extra_values.split(",") if v.strip()]:
                        extra_base_node = extra_value
                        # Create only if missing
                        if extra_base_node not in G.nodes:
                            G.add_node(extra_base_node, color='orange', edgecolor='black', position='center')

                        # Link triples/opinion nodes to this extra base node
                        for aspect, opinion, sentiment in triples:
                            if aspect not in G.nodes:
                                G.add_node(aspect, color='yellow', edgecolor='blue', position='none')
                            if opinion not in G.nodes:
                            ...
                            # Edge: aspect -> opinion (sentiment)
                            if not G.has_edge(aspect, opinion):
                                G.add_edge(aspect, opinion, label=sentiment, color=color.get(sentiment))

                            # Optional direct edge: extra base -> opinion
                            if not G.has_edge(extra_base_node, opinion):
                                G.add_edge(extra_base_node, opinion, label="corresponds to", color='orange')
    ...
    def render_triple(triple, extras=None):
        word, label, sentiment = triple
        color = {"pos": "#afa", "neg": "#faa", "neu": "#d3d3d3"}.get(sentiment, "#fff")

        # show colored triple
        annotated_text((word, label, color))

        # small inline plain text for extra categories
        if extras:
            extras_txt = ", ".join(extras)
            st.markdown(
                f"<span style='font-size:0.9em; color:#666; margin-left:6px;'>"
                f"(also: {extras_txt})"
                f"</span>",
    ...
    with tab0:
        ...
        for MAIN_CATEGORY in subset["MAIN_CATEGORY"].unique():
            st.title(MAIN_CATEGORY)

            raw_base_categories = subset[subset["MAIN_CATEGORY"] == MAIN_CATEGORY]["base_category"].dropna().unique()

            # Get first categories only for expander
            first_categories = {str(bc).split(",")[0].strip() for bc in raw_base_categories}

            for category in first_categories:

                # Filter rows where first base category matches this expander
                subcf = subset[
                    (subset["MAIN_CATEGORY"] == MAIN_CATEGORY) &
                    (subset["base_category"].str.split(",").str[0].str.strip() == category)
                ]
                ...
                with st.expander(category, expanded=False):
                    ...
                    for _, row in subcf.iterrows():
                        # Split all base categories
                        parts = [p.strip() for p in str(row["base_category"]).split(",")]

                        # Include EXTRA Opinion if present and not FALSE
                        extra_col = "EXTRA_Opinion_Base_Needed_RARE_EXAMPLE"
                        extra_val = row.get(extra_col, "FALSE")
                        if pd.notna(extra_val) and extra_val != "FALSE":
                            parts.append(extra_val.strip())

                        # Remove first category (already used for expander)
                        extras = parts[1:] if len(parts) > 1 else None

                        render_triple(row["triple"], extras)
    ...
    with tab01:
        ...
        options_with_all = ["All"] + all_main_categories

        selected_main_categories = st.multiselect(
            "Select Main Categories to Display",
            options=options_with_all,
            default=all_main_categories,
            key="MAINCATEGORY_subj"
        )

        # --- Exclusive "All" logic for main categories ---
        if "All" in selected_main_categories and len(selected_main_categories) == 1:
            selected_main_categories_filtered = [x for x in all_main_categories if x != "All"]
        elif "All" in selected_main_categories and len(selected_main_categories) > 1:
            selected_main_categories.remove("All")
            selected_main_categories_filtered = [x for x in selected_main_categories if x != "All"]
        else:
            selected_main_categories_filtered = [x for x in selected_main_categories if x != "All"]

        # --- Filter subset for selected MAIN_CATEGORY first ---
        if selected_main_categories_filtered:
            main_filtered_subset = subset[subset["MAIN_CATEGORY"].isin(selected_main_categories_filtered)]
        else:
            main_filtered_subset = subset.copy()  # no main category selected → keep all

        # --- Flatten base categories per row (including extra opinions) ---
        extra_col = "EXTRA_Opinion_Base_Needed_RARE_EXAMPLE"

        def get_row_base_categories(row):
            # Only include the real base categories
            base_cats = [c.strip() for c in row["base_category"].split(",")] if pd.notna(row["base_category"]) else []
            return base_cats


        main_filtered_subset["all_base_categories"] = main_filtered_subset.apply(get_row_base_categories, axis=1)

        # --- Collect unique base categories from filtered MAIN_CATEGORY rows ---
        all_base_cats_flat = [cat for cats in main_filtered_subset["all_base_categories"] for cat in cats]
        base_categories = list(dict.fromkeys(all_base_cats_flat))  # preserve order

        # --- Multiselect for base categories ---
        base_options_with_all = ["All"] + base_categories
        selected_base_categories = st.multiselect(
            "Select Base Part to Display",
            options=base_categories,
            default=base_categories,
            key="BASECATEGORY_subj"
        )

        # --- Exclusive "All" logic for base categories ---
        if "All" in selected_base_categories and len(selected_base_categories) == 1:
            final_base_selection = [x for x in base_categories if x != "All"]
        else:
            final_base_selection = [x for x in selected_base_categories if x != "All"]

        # --- Filter rows: only those matching selected base categories ---
        if final_base_selection:
            filtered_subset = main_filtered_subset[
                main_filtered_subset["all_base_categories"].apply(lambda cats: bool(set(cats) & set(final_base_selection)))
            ]
        else:
            filtered_subset = main_filtered_subset.copy()  # no base selection → keep all

        density = st.slider("Graph Density", 1, 10, value=1, key="density1_subj")
        ...
        G, fig = base_entity_graph(car_id, filtered_subset, density,
                                   selected_main_categories_filtered, final_base_selection)
        st.pyplot(fig)
        ...
        st.download_button(
            ...
            key=f"d1_subj_final1_{car_id}",
            data=png_buffer,
            file_name=f"car_{car_id}_graph.png",
            mime="image/png"
        )

        # Make graph safe for GraphML
        for n, data in G.nodes(data=True):
            for key, value in data.items():
                if value is None:
                    if key in ["color", "edgecolor", "position"]:
                        data[key] = "none"
                    else:
                        data[key] = ""
        ...
        st.download_button(
            ...
            key=f"d2_subj_prelim1_{car_id}",
            data=graphml_buffer,
            file_name=f"car_{car_id}_graph.graphml",
    ...
    with tab1:
        ...
        if f=='One-by-One': 
            ...
            #'...and now we\'re done!
            triples=get_triples(topicnames.index(topic),df,car_id,'all','all')
            if len(triples)==0:
                st.write('No triples related to this topic')
            
            #st.write(sentiment_distribution(p,n))
            else:
                fig=draw_topic_graph(topic,df,car_id)
                if fig:
                    st.pyplot(fig)
                #st.write(sentiment_distribution(p,n))
                ...
                if radio!=None:
                    fig=draw_topic_graph(topic,df,car_id,radio, 'all')
                    if fig:
                        st.pyplot(fig)
        if f=='All':
            st.title('Topics')
            ...
            for topicn in topicnames:
                #st.header(topicn)
                subset=df.loc[(df["id"]==car_id) & (df["sentiment"]!='-') & (df["label_topic"]==topicnames.index(topicn))]
                triples=list(subset["triple"])
                ...
                neu=len([t[2] for t in triples if t[2]=='neu'])
    ...
    with tab2:
        ...
        if f1=='One-by-One': 
            ...
            for i in range(100):
              ...
              time.sleep(0.001)
            '..now we are done! 3 2 1....'
            fig=draw_sent_graph(df,car_id,'all', radio, flag=1)
            if fig:
                st.pyplot(fig)
            topic=st.selectbox("Filter by Topic", topicnames, index=None, key='5_subj')
            if topic:
                fig=draw_topic_graph(topic,df,car_id,'all', radio, flag=0)
                if fig:
                    st.pyplot(fig)
        ...
        if f1=='All':
            ...
            for topic in range(0,10,1):
                triples=get_triples(topic,df,car_id,sentiment='pos')
                p=p+len(triples)
                triples=get_triples(topic,df,car_id,sentiment='neg')
                n=n+len(triples)
    ...
    with tab3:
        density = st.slider("Graph Density", 1, 10, value=1,key="density2_subj")
        'Building Knowledge Graph...'
        ...
        G, fig = entity_graph(car_id, df, density)
        st.pyplot(fig)
        st.title('Download Graph')
        ...
        st.download_button(
            label="Download Graph as PNG",
            key=f"d1_subj_final2_{car_id}",  # ✅ unique key
            data=png_buffer,
            file_name=f"car_{car_id}_graph.png",
            mime="image/png"
            )
        ...
        st.download_button(
            label="Download Graph as GraphML",
            key=f"d2_subj_prelim2_{car_id}",  # ✅ unique key
            data=graphml_buffer,
            file_name=f"car_{car_id}_graph.graphml",
            mime="application/graphml+xml"
            )
    ...
    with tab4:
        f = st.segmented_control("Filter", ["One-by-One", "All"], key="filter_subj_Control_aspect", default="All")

        if f=='One-by-One': 
            #topic=st.selectbox("Choose Apect", aspects, key=3)
            
            
            aspects = list(set(df.loc[df['id'] == car_id, 'aspect']))
            ...
            p=len([t[2] for t in triples if t[2]=='pos'])
            n=len([t[2] for t in triples if t[2]=='neg'])
            fig=draw_topic_graph(topic,df,car_id,selected_aspect, 'all', flag=1)
            if fig:
                st.pyplot(fig)
        ...
        if f == 'All':
            ...
            id = car_id
            aspects = list(set(df.loc[df['id'] == car_id, 'aspect']))
            opinions = list(set(df.loc[df['id'] == car_id, 'opinion']))
            if "-" in aspects:
                aspects.remove("-")
            st.title(f"Total Number of Aspects: {len(aspects)}")
            ...
            for aspect in filtered_aspects:
                subset = cf.loc[
                    (cf["id"] == id) &
                    (cf["aspect"] == aspect) &
                    (cf["sentiment"].isin(selected_sentiments))
                ]
                triples = list(subset["triple"])
                if len(triples) > 0:
            ...
            for i in range(0, len(display_aspects), num_columns):
                        ...
                        for j, aspect in enumerate(display_aspects[i:i+num_columns]):
                                        with row[j]:
                                            # Get the subset for this aspect with the selected sentiments
                                            subset_aspect = cf[
                                                        (cf["id"] == id) & 
                                                        (cf["aspect"] == aspect) & 
                                                        (cf["sentiment"].isin(selected_sentiments))
                                            ...
                                            if subset_aspect.empty:
                                                        continue

                                            # Get main/base category for this aspect
                                            main_category = subset_aspect["MAIN_CATEGORY"].iloc[0]
                                            base_category = subset_aspect["base_category"].iloc[0]

                                            # Group opinions by topic
                                            topic_dict = {}
                                            for _, row_topic in subset_aspect.iterrows():
                                                        topic = topicnames[int(row_topic["label_topic"])]
                                                        opinion, sentiment = row_topic["opinion"], row_topic["sentiment"]
                                                        if topic not in topic_dict:
                                                                    topic_dict[topic] = []
                                                        topic_dict[topic].append((opinion, sentiment, row_topic))  # keep row_topic for extra column

                                            # Build HTML card
                                            html_card = "<div style='border:1px solid #ddd; padding:10px; border-radius:5px; margin-bottom:10px;'>"
                                            html_card += f"<div style='font-size:12px; color:#666; margin-bottom:5px;'>{main_category} | {base_category}</div>"
                                            html_card += f"<h4 style='margin:0 0 5px 0'>{aspect}</h4>"

                                            # Add opinions, one per line, with extra info
                                            for topic, opinions in topic_dict.items():
                                                        html_card += f"<div style='margin-bottom:5px;'><b>{topic}</b>:</div>"
                                                        for opinion, sentiment, row_topic in opinions:
                                                                    ...
                                                                    # Each opinion in its own div
                                                                    html_card += f"<div style='margin-left:10px; color:{color}'>{opinion}</div>"

                                                                    # Extra info immediately below the opinion
                                                                    extra_col = "EXTRA_Opinion_Base_Needed_RARE_EXAMPLE"
                                                                    extra_value = row_topic.get(extra_col, "FALSE")
                                                                    if pd.notna(extra_value) and extra_value != "FALSE":
                                                                                html_card += f"<div style='margin-left:20px; font-size:12px; color:#999; margin-top:2px;'>corresponds to: {extra_value}</div>"
    ...
    with tab5:
        ...
        # ------------------ NEW SECTION ------------------
        st.subheader("🔍 View a Snippet by Segment ID")

        # Get unique segment IDs for this car from cf
        segment_ids = cf[cf["id"] == car_id]["segment_id"].dropna().unique().tolist()
        if segment_ids:
            selected_segment_id = st.selectbox("Select Segment ID", options=segment_ids,key='reviewX_subj')

            if selected_segment_id:
                # Filter cf for this car_id and segment_id
                snippet_rows = cf[(cf["id"] == car_id) & (cf["segment_id"] == selected_segment_id)]

                if not snippet_rows.empty:
                    # Display the review text (assume same for all rows for this segment)
                    review_text_for_segment = snippet_rows["review"].iloc[0] if "review" in snippet_rows.columns else snippet_rows["text"].iloc[0]
                    st.markdown(f"""
                        <div style="
                            ...
                            line-height:1.5;
                        ">
                            {escape(review_text_for_segment)}
                        </div>
                    """, unsafe_allow_html=True)

                    # Display all triples for this segment
                    st.markdown("**Extracted Triples for this Segment:**")
                    for _, triple_row in snippet_rows.iterrows():
                        aspect, opinion, sentiment = triple_row["aspect"], triple_row["opinion"], triple_row["sentiment"]
                        st.markdown(f"- **Aspect:** {aspect} | **Opinion:** {opinion} | **Sentiment:** {sentiment}")

                else:
                    st.info("No data found for this segment ID.")
        else:
            st.info("No segments found for this car.")
