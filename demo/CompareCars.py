...
def app():
    ...
    criterion=st.selectbox('Compare cars on:',options= ['Specifications', 'Phrases','Aspect', 'Sentiment'], key='comparecars_comp')
    ...
    if criterion=='Sentiment':
        ...
        with col1:
            ...
            subset=df.loc[(df["id"]==st.session_state["car_1_comp"]) & (df["sentiment"]=='pos')]
            triples=list(subset["triple"])
            p1=len(triples)
            subset=df.loc[(df["id"]==st.session_state["car_2_comp"]) & (df["sentiment"]=='neg')]
            triples=list(subset["triple"])
            n1=len(triples)
        ...
        with col1:
            #for topic in range(0,10,1):
                #st.header('Negatives')
            subset=df.loc[(df["id"]==st.session_state["car_1_comp"]) & (df["sentiment"]=='neg')]
            triples=list(subset["triple"])#p=p+len(triples)
    ...
    if criterion=='Aspect':
        ...
        with col1:
            ...
            if "-" in aspects:
                aspects.remove("-")
            st.title(f"Total Number of positive or  negative Aspects: {len(aspects)}")
