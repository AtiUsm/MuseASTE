...
def app():
    ...
    criterion=st.selectbox('Compare cars on:',options= ['Specifications', 'Phrases','Aspect', 'Sentiment', 'Topic', 'Structural Base Category'], key='comparecars_comp')
    ...
    if criterion == 'Structural Base Category':
        ...
        all_main_categories = sorted(cf["MAIN_CATEGORY"].dropna().unique().tolist())

        selected_main_categories = st.selectbox(
            "Select Base Categories to Display",
            options=all_main_categories,
            index=6,
            key="MAINCATEGORY_comp"
        )
        
        # --------------------- CAR 1 ---------------------
        with col1:
            subset = cf[(cf["id"] == car_id_1) & (cf["sentiment"] != '-')]
            count = 0
            MAIN_CATEGORY = selected_main_categories
            ...
            for category in first_categories:
                ...
                with st.expander(category, expanded=False):
                    for _, row in subcf.iterrows():
                        ...
                        # Include EXTRA Opinion if present
            ...
            if count == 0:
                st.write("No triples Found")

        # --------------------- CAR 2 ---------------------
        with col2:
            subset = cf[(cf["id"] == car_id_2) & (cf["sentiment"] != '-')]
            count = 0
            MAIN_CATEGORY = selected_main_categories
            st.title(MAIN_CATEGORY)
